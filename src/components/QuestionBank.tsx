...
export const QuestionBank: React.FC<QuestionBankProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Grade Coverage Ribbon (Verification of >= 30 questions per grade) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sortedFilteredQuestions.map((q, idx) => {
          ...
          return (
            ...
            >
              <div className="space-y-3">
                {/* Header row: Code, Skill badge, Difficulty badge */}
      ...
      {/* ========================================================================= */}
      {/* PRINT-ONLY TEMPLATE (Strictly adheres to user's filtering & sorting choice) */}
      {/* ========================================================================= */}
      <div 
        className="hidden print:block fixed inset-0 z-[9999] bg-white text-black p-8 overflow-y-auto ltr:text-left rtl:text-right" 
        dir="rtl"
        style={{ fontFamily: "'Cairo', 'Tajawal', sans-serif" }}
      >
        {/* Official Header */}
        <div className="border-b-2 border-slate-900 pb-5 mb-6 flex items-center justify-between">
          <div className="space-y-1 text-xs font-bold leading-relaxed text-slate-800">
            <div>المملكة العربية السعودية</div>
            <div>وزارة التعليم</div>
            <div>الإدارة العامة للتعليم بمنطقة الرياض</div>
            <div className="font-extrabold text-sky-800 text-sm">{settings?.schoolName || 'مدارس رواد الموهبة الأهلية'}</div>
            <div>قسم رعاية الموهوبين والمبدعين</div>
          </div>

          <div className="text-center space-y-1">
            <div className="w-16 h-16 mx-auto rounded-xl border border-slate-300 flex items-center justify-center p-2 bg-slate-50">
              <SchoolLogo size={48} />
            </div>
            <div className="text-[11px] font-black tracking-wide text-slate-900">
              {settings?.platformName || 'منصة الموهوبين'}
            </div>
          </div>

          <div className="space-y-1 text-xs text-slate-700 text-left" dir="ltr">
            <div className="font-mono text-[11px]">DATE: {new Date().toLocaleDateString('ar-SA')}</div>
            <div className="font-mono text-[11px]">TIME: {new Date().toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}</div>
            <div className="font-bold text-slate-900 text-right" dir="rtl">
              منسق الموهوبين: أسامة ابراهيم
            </div>
            <div className="text-[10px] text-slate-500 text-right" dir="rtl">
              المشرف: {settings?.supervisorName || 'أ. أسامة ابراهيم'}
            </div>
          </div>
        </div>

        {/* Title & Filter Criteria Summary Box */}
        <div className="bg-slate-50 border-2 border-slate-800 rounded-2xl p-4 mb-6">
          <div className="flex items-center justify-between mb-3 border-b border-slate-300 pb-2">
            <div>
              <h1 className="text-xl font-black text-slate-900">
                وثيقة مراجعة بنك الأسئلة المقننة
              </h1>
              <p className="text-xs text-slate-600 font-medium">
                تم استخراج الأسئلة وفق معايير الفرز والتصفية المحددة من قبل المشرف لسهولة المراجعة والتدقيق
              </p>
            </div>
            <div className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-xs font-black">
              الأسئلة المصدرة: {questionsToExport.length} سؤالاً
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <div className="p-2 rounded bg-white border border-slate-200">
              <span className="block text-[10px] text-slate-500 font-bold">الصف المستهدف:</span>
            ...
            <div className="p-2 rounded bg-white border border-slate-200">
              <span className="block text-[10px] text-slate-500 font-bold">مستوى الصعوبة المطبق:</span>
              <span className="font-extrabold text-slate-900">
            ...
            <div className="p-2 rounded bg-white border border-slate-200">
              <span className="block text-[10px] text-slate-500 font-bold">المهارة العقلية:</span>
              <span className="font-extrabold text-slate-900">
                {selectedSkill === 'all' ? 'كافة المهارات العقلية' : SKILL_DEFINITIONS[selectedSkill as SkillCategory]?.name}
              </span>
            </div>

            <div className="p-2 rounded bg-white border border-slate-200">
              <span className="block text-[10px] text-slate-500 font-bold">ترتيب وتنظيم الأسئلة:</span>
              <span className="font-extrabold text-slate-900">
                {sortBy === 'difficulty_asc' ? 'من الأسهل للأصعب (متوازن)' :
                 sortBy === 'difficulty_desc' ? 'من الأصعب للأسهل' :
                 sortBy === 'skill' ? 'حسب المهارة العقلية' :
                 sortBy === 'grade' ? 'حسب الصف الدراسي' : 'تسلسلي بالرمز'}
              </span>
            </div>
          </div>

          {/* Balance Breakdown Bar */}
          <div className="mt-3 pt-2 border-t border-slate-200 flex items-center justify-between text-xs flex-wrap gap-2">
            <span className="font-bold text-slate-700">توزيع التوازن في هذا التقرير:</span>
            <div className="flex items-center gap-3 font-mono font-bold">
              <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                سهل: {exportStats.easy}
              </span>
              <span className="text-sky-700 bg-sky-50 px-2 py-0.5 rounded border border-sky-200">
                متوسط: {exportStats.medium}
              </span>
              <span className="text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                صعب: {exportStats.hard}
              </span>
              {exportStats.advanced > 0 && (
                <span className="text-purple-700 bg-purple-50 px-2 py-0.5 rounded border border-purple-200">
                  متقدم: {exportStats.advanced}
                </span>
              )}
            </div>
          </div>
        ...
        {exportGrouping === 'difficulty' && (
          <div className="space-y-8">
            {(['easy', 'medium', 'hard', 'advanced'] as DifficultyLevel[]).map(diffKey => {
              ...
              return (
                <div key={diffKey} className="break-inside-avoid-page space-y-4">
                  <div className="p-2.5 rounded-xl bg-slate-100 border-r-4 border-slate-900 flex items-center justify-between">
        ...
        {exportGrouping === 'skill' && (
          <div className="space-y-8">
            {(Object.entries(SKILL_DEFINITIONS) as [SkillCategory, any][]).map(([skillKey, def]) => {
              ...
              return (
                <div key={skillKey} className="break-inside-avoid-page space-y-4">
                  <div className="p-2.5 rounded-xl bg-slate-100 border-r-4 border-slate-900 flex items-center justify-between" style={{ borderColor: def.color }}>
                    <h2 className="text-base font-black text-slate-900">
        ...
        <div className="mt-12 pt-6 border-t-2 border-slate-900 break-inside-avoid-page">
          <div className="grid grid-cols-3 gap-6 text-center text-xs font-bold">
            <div className="space-y-6">
              <div className="text-slate-700">إعداد وتنسيق ومراجعة</div>
              <div className="font-extrabold text-slate-900 text-sm">منسق الموهوبين أسامة ابراهيم</div>
              <div className="text-[10px] text-slate-400">التوقيع: .......................................</div>
            </div>

            <div className="space-y-6">
              <div className="text-slate-700">الإشراف الفني والتربوي</div>
              <div className="font-extrabold text-slate-900 text-sm">{settings?.supervisorName || 'أ. أسامة ابراهيم'}</div>
              <div className="text-[10px] text-slate-400">التوقيع: .......................................</div>
            </div>

            <div className="space-y-6">
              <div className="text-slate-700">الاعتماد الرسمي بالمدرسة</div>
              <div className="font-extrabold text-slate-900 text-sm">مدير المدرسة / قائد المجمع</div>
              <div className="text-[10px] text-slate-400">الختم الرسمي: [ ......................... ]</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ========================================================================= */}
      {/* EXPORT CUSTOMIZATION MODAL (Allows supervisor to review & configure PDF)  */}
      {/* ========================================================================= */}
      {isExportModalOpen && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-slate-500 mb-2">الصفوف المستهدفة (يمكنك اختيار أكثر من صف)</label>
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-slate-500">
                أدخل الأسئلة (سطر لكل سؤال) بالصيغة التالية:
                <br />
                <span className="text-sky-600">نص السؤال | خيار1, خيار2, خيار3 | رقم الإجابة (0-2) | [الصعوبة: easy/medium/hard] | [المهارة]</span>
              </label>
              <textarea
                rows={8}
                value={bulkInputText}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {/* Supervisor Notes */}
              {previewQuestion.supervisorNotes && (
                <div className="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 text-xs text-slate-600 dark:text-slate-400">
                  <span className="font-bold text-slate-700 dark:text-slate-300">ملاحظات المشرف:</span> {previewQuestion.supervisorNotes}
                </div>
              )}
            </div>
          </div>
...
/**
 * Printable Question Card Component
 * Formatted cleanly for paper / PDF export
 */
...
const PrintQuestionCard: React.FC<PrintQuestionCardProps> = ({
  ...
  return (
    <div className="border-2 border-slate-300 rounded-xl p-4 break-inside-avoid-page space-y-3 bg-white">
      {/* Question Header */}
      <div className="flex items-center justify-between border-b border-slate-200 pb-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="bg-slate-900 text-white font-black px-2 py-0.5 rounded text-xs">
            سؤال {index}
          </span>
        ...
        <div className="flex items-center gap-2">
          <span className="font-bold text-slate-700 border border-slate-300 px-2 py-0.5 rounded">
            {diffInfo.label}
          </span>
      ...
      {(question.svgGraphic || question.imageUrl) && (
        <div className="p-3 border border-slate-200 rounded-lg flex justify-center bg-slate-50 my-2">
          {question.svgGraphic && (
            <VisualShape type={question.svgGraphic} size={180} />
          )}
          ...
          return (
            <div
              ...
              className={`p-2.5 rounded-lg border text-xs flex items-center justify-between ${
                isCorrect 
                  ? 'border-2 border-emerald-700 bg-emerald-50 text-emerald-950 font-black' 
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {opt.svgShape && (
                  <div className="mr-2">
                    <VisualShape type={opt.svgShape} size={36} />
                  </div>
                )}
              </div>

              {isCorrect && (
                <span className="text-[10px] text-emerald-800 font-black mr-2">
                  ✓ الإجابة الصحيحة
                </span>
              )}
            </div>
      ...
      {includeExplanations && question.explanation && (
        <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-700 space-y-0.5 mt-2">
          <span className="font-black text-slate-900 block">تفسير النمط العقلي والحل المعتمد:</span>
